...
# ===============================================================================
# Restaurant Management System Assignment - Batch Grading Script
# ===============================================================================
# This script processes all student submissions and generates grade files

echo "=========================================================================="
echo "        Restaurant Management System Assignment - Batch Grading"
echo "=========================================================================="
echo "Date: $(date)"
...
if [ ! -f "autograder_restaurant_management_system.sh" ]; then
    echo "❌ ERROR: autograder_restaurant_management_system.sh not found in current directory!"
...
REQUIRED_FRAMEWORK_FILES=("driver.c" "restaurant.h" "Makefile" "TESTCASES.txt" "EXPECTED_OUTPUT.txt")
...
chmod +x autograder_restaurant_management_system.sh
...
echo "Restaurant Management System Assignment - Grading Summary" > "$SUMMARY_FILE"
...
REQUIRED_C_FILES=("copyString.c" "findMenuItem.c" "calculateSum.c" "addMenuItem.c" "processOrder.c" "sortMenu.c")
...
extract_student_name() {
    ...
    if [[ "$base_name" =~ ^[Rr]estaurant.*[_\-\.]([A-Za-z]+[_\-\s]+[A-Za-z]+) ]]; then
        # Pattern: Restaurant_Assignment_FirstName_LastName
        echo "${BASH_REMATCH[1]}" | tr '_-' ' '
    elif [[ "$base_name" =~ ^[Aa]ssignment.*[_\-\.]([A-Za-z]+[_\-\s]+[A-Za-z]+) ]]; then
        # Pattern: Assignment3_FirstName_LastName
        echo "${BASH_REMATCH[1]}" | tr '_-' ' '
    elif [[ "$base_name" =~ ^([A-Za-z]+[_\-\s]+[A-Za-z]+)[_\-\.].*[Aa]ssignment ]]; then
        # Pattern: FirstName_LastName_Assignment3
        echo "${BASH_REMATCH[1]}" | tr '_-' ' '
...
if [ ${#zip_files[@]} -eq 0 ]; then
    ...
    echo "Supported format: Any .zip file containing the 6 required C implementation files"
    exit 1
fi
...
for zip_file in "${zip_files[@]}"; do
    ...
    # Generate grade report header
    {
        ...
        echo "Graded on: $(date)"
        echo "Graded by: Restaurant Management System Autograder"
        echo
    ...
    AUTOGRADER_OUTPUT=$(timeout 15 ./autograder_restaurant_management_system.sh 2>&1)
    ...
    if [ $AUTOGRADER_EXIT_CODE -eq 0 ]; then
        ...
        # Also extract individual function scores for detailed analysis
        COMPILATION_STATUS=$(echo "$AUTOGRADER_OUTPUT" | grep "Compilation & Build:" | grep -o '[0-9]\+/[0-9]\+')
        ...
        echo "- Missing pointer increment statements (dest++, src++)" >> "$GRADE_FILE"
        ...
        echo "- Check copyString, findMenuItem, calculateSum, and sortMenu functions" >> "$GRADE_FILE"
        ...
        echo "- Infinite loops (missing pointer increments like dest++, src++)" >> "$GRADE_FILE"
    ...
    rm -f STUDENT_OUTPUT.txt restaurant *.o 2>/dev/null
...
echo >> "$SUMMARY_FILE"
echo "Note: Manual grading (10 points) for code quality and comments" >> "$SUMMARY_FILE"
echo "still needs to be added to autograder scores." >> "$SUMMARY_FILE"
